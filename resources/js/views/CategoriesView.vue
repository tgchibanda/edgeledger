  ...
  <AppLayout>
    <div class="p-6 max-w-4xl">
      <div class="page-header">
        ...
        <p class="page-sub">Manage your {{ TF.h4 }}, {{ TF.m15 }} and {{ TF.m1 }} market structure categories.</p>
      </div>

      <div v-for="tf in ['H4','M15','M1']" :key="tf" class="card mb-5">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-base font-semibold" :class="tf==='H4'?'text-blue-400':tf==='M15'?'text-purple-400':'text-yellow-400'">
            {{ tfLabel(tf) }} Categories
          </h2>
          <button class="btn-primary btn-sm" @click="openAdd(tf)">+ Add</button>
        </div>
        <div class="space-y-2">
          <div v-for="c in catsByTf[tf]" :key="c.id"
            class="flex items-center justify-between px-3 py-2 bg-surface rounded-lg border border-border">
            <div>
          ...
          <div v-if="!catsByTf[tf].length" class="text-sm text-gray-600 text-center py-4">
            No {{ tfLabel(tf) }} categories yet.
          </div>
        </div>
      </div>
        ...
        <div class="card w-full max-w-md">
          ...
          <div class="form-group">
            ...
            <select v-model="modal.timeframe" class="select" :disabled="!!modal.id">
              <option value="H4">{{ TF.h4 }}</option>
              <option value="M15">{{ TF.m15 }}</option>
              <option value="M1">{{ TF.m1 }}</option>
            </select>
          </div>
          ...
          <div class="flex gap-3">
            <button class="btn-primary" :disabled="modal.saving" @click="saveCategory">
...
export default {
  ...
  computed: {
    catsByTf() {
      return {
        H4:  this.cats.filter(c => c.timeframe === 'H4'),
        M15: this.cats.filter(c => c.timeframe === 'M15'),
        M1:  this.cats.filter(c => c.timeframe === 'M1'),
      }
    },
  },
  ...
  methods: {
    tfLabel(tf) {
      return tf === 'H4' ? this.TF.h4 : tf === 'M15' ? this.TF.m15 : this.TF.m1
    },
    async load() {
    ...
    openAdd(tf) {
      this.modal = { show:true, id:null, timeframe:tf, name:'', description:'', saving:false, error:'' }
    },
    openEdit(c) {
      this.modal = { show:true, id:c.id, timeframe:c.timeframe, name:c.name, description:c.description||'', saving:false, error:'' }
    },
    async saveCategory() {
      ...
      try {
        ...
        this.modal.error = 'Failed to save.'
